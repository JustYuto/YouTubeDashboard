    ...
    <div class="row">
      ...
      <div class="col-md-6 canvas-container">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <button @click="resetCharts" class="btn btn-custom">
            Reset Charts
          </button>
        </div>
        <canvas
          ref="ageGroupPieChart"
          id="ageGroupPieChart"
          width="400"
          height="300"
        ></canvas>
...
export default {
  ...
  methods: {
      ...
      this.analyticsData3.rows.forEach((row) => {
        ...
        if (gender !== "genderUserSpecified") {
          ...
          if (ageGroupData[ageGroup]) {
            ageGroupData[ageGroup][gender] =
              (ageGroupData[ageGroup][gender] || 0) + viewerPercentage;
          } else {
    ...
    renderCityWatchChart(ctx) {
      ...
      const averageViewDurationData = this.analyticsData2.rows.map(
        (row) => row[2]
    ...
    renderAgeGroupPieChart() {
      ...
      this.ageGroupPieChart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [
            {
              data: Object.values(this.ageGroupData).map(
                (data) => data.male + data.female
              ),
              backgroundColor: this.colors.slice(
                0,
                Object.keys(this.ageGroupData).length
              ),
            },
          ],
        },
        options: {
          ...
          onClick: (evt, item) => {
            if (item.length > 0) {
              const selectedAgeGroup = Object.keys(this.ageGroupData)[
                item[0].index
              ];
    ...
    filterCharts(filterType, filterValue) {
      ...
      if (filterType === "gender") {
        ...
        Object.keys(this.ageGroupData).forEach((ageGroup) => {
          newAgeGroupData[ageGroup] =
            this.ageGroupData[ageGroup][filterValue] || 0;
        });
        this.ageGroupPieChart.data.datasets[0].data =
          Object.values(newAgeGroupData);
        ...
        this.genderPieChart.data.datasets[0].data = [
          newGenderData.male,
          newGenderData.female,
        ];
    ...
    renderSubscribersChart() {
      ...
      const data = this.analyticsData4.rows.filter(
        (row) => row[2] !== 0 || row[3] !== 0
      );
